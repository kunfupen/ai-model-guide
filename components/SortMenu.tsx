...
export function SortMenu() {
  ...
  return (
    <details className="group relative">
      <summary className="flex cursor-pointer list-none items-center gap-2 rounded-full border border-zinc-200 bg-white px-3 py-1 text-xs font-medium text-zinc-700 transition-colors hover:border-zinc-400 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-300 dark:hover:border-zinc-600 [&::-webkit-details-marker]:hidden">
      ...
      <ul className="absolute right-0 z-20 mt-2 min-w-[12rem] rounded-lg border border-zinc-200 bg-white py-1 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        {SORT_OPTIONS.map((o) => (
          <li key={o.key}>
            <button
              type="button"
